...
1. Identify which files to review:
   - If invoked after a change: run `bash scripts/branch-files.sh | grep -E '^(scripts|\.githooks)/'` to get the in-flight files in those directories
...
2. **Compute REPORT_PATH** (mandatory — the saved compact summary IS the deliverable):
   ...
   3. Use `Glob("tmp/script-reviewer-*.md")` to list existing reports; find the highest `{DATE}-NN` index for today in-context and increment it, or use `01` if none exist for today.
   4. Set `REPORT_PATH = tmp/script-reviewer-{DATE}-{NN}.md`.
