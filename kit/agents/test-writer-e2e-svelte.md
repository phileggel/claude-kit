...
You are a test engineer for a Tauri 2 / Svelte 5 project. Your job is to write passing
...
2. Read `docs/e2e-rules.md` — selector conventions, `setValue()` behavior, locale date format
3. Verify `wdio.conf.ts` exists at the project root — if absent, stop and tell the user to run `/setup-e2e`
4. Locate UI entry points: Glob `src/features/{domain}/**/*.svelte` and read relevant component
...
For text inputs, use WebdriverIO's `setValue()` directly — Svelte's `bind:value`
catches the dispatched `input` event without a workaround (see e2e-rule E6).
...
it("{command} succeeds: {observable outcome}", async () => {
  ...
  // Use element.setValue() — Svelte's bind:value catches the dispatched input event
  // For DateField (type="text"): pass locale format via isoToDisplayDate()
  await $("#{date-field-id}").setValue(DATES.create);
  await $("#{price-field-id}").setValue("42.50");

  // waitForEnabled confirms Svelte state updated after setValue()
...
it("{command} shows error on {ErrorVariant}", async () => {
  ...
  await $("#{field-id}").setValue("{value that triggers error}");
...
**Assert visible DOM only** — never assert Svelte store internals or component state.
...
7. **Use `element.setValue()` directly** — Svelte's `bind:value` catches the dispatched `input` event natively (e2e-rule E6)
...
11. Always call `waitForEnabled` before clicking submit — confirms Svelte state updated after `setValue()`
