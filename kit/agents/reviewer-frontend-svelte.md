...
No argument required. The agent discovers changed `.ts` / `.svelte` files under `src/` via `bash scripts/branch.sh files`. E2E test files under `e2e/` are excluded — they're `reviewer-e2e`'s lane.
