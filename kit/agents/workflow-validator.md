...
3. Use `Glob("tmp/workflow-validator-*.md")` to list existing reports; find the highest `{DATE}-NN` index for today in-context and increment it, or use `01` if none exist for today.
4. Set `REPORT_PATH = tmp/workflow-validator-{DATE}-{NN}.md`.
...
- Otherwise: run `bash scripts/branch-files.sh`, infer the feature domain from the modified file paths, then search for a matching file via `Glob docs/plan/*-plan.md`.
...
Run `bash scripts/branch-files.sh`. Determine which conditional items in the plan are actually required:
