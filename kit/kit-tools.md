...
| `reviewer-frontend-svelte`    | Any `.ts` / `.svelte` modified in Svelte projects — synced as `reviewer-frontend`                | Svelte 5 variant of `reviewer-frontend`. Pivots from React hooks (`useCallback`/`useMemo`) to Svelte 5 runes correctness (`$state` placement, uncleaned `$effect`, illegal mutation patterns). UX/M3/E2E sections transfer unchanged.                                                                                      |
