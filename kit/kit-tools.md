...
| `reviewer-frontend-svelte`    | Any `.ts` / `.svelte` under `src/` modified in Svelte projects — synced as `reviewer-frontend`   | Svelte 5 variant of `reviewer-frontend`. Pivots from React hooks (`useCallback`/`useMemo`) to Svelte 5 runes correctness (`$state` placement, uncleaned `$effect`, illegal mutation patterns). UX/M3/E2E sections transfer unchanged. Scoped to `src/` — E2E test files live in `reviewer-e2e`'s lane                      |
...
| `reviewer-security-svelte`    | Same trigger in Svelte projects — synced as `reviewer-security`                                  | Svelte 5 variant. Frontend XSS shifts from `dangerouslySetInnerHTML` to `{@html}` (the auto-escape bypass). Scans `.svelte` files. Storage misuse adapts to `$state`/stores.                                                                                                                                               |
...
| `test-writer-frontend-svelte` | After backend commit, before frontend impl in Svelte projects — synced as `test-writer-frontend` | Svelte 5 variant of `test-writer-frontend`. Uses `@testing-library/svelte` instead of RTL; drops `renderHook` (reactive `.svelte.ts` modules are tested directly with `flushSync()`).                                                                                                                                      |
...
| `test-writer-e2e-svelte`      | Phase 4 (quality) for Svelte projects — synced as `test-writer-e2e` downstream                   | Svelte 5 variant of `test-writer-e2e`. Drops the React `setReactInputValue` workaround; templates call `$("#id").setValue(...)` directly. `sync-config.sh` strips the `-svelte` suffix at sync time.                                                                                                                       |
