...
class QualityChecker:
    ...
    def _run_frontend_group(self):
        ...
        if run_tests:
            if not self._maybe_skip_for_stack(
                "react_tests", "Frontend Tests", self.package_json, SKIP_FRONTEND_ABSENT
            ):
                # --passWithNoTests: a scaffolded frontend stack with no test
                # files yet (mid-bootstrap, fresh feature start) would
                # otherwise exit 1 from vitest's "No test files found" path
                # and fail the suite. The flag short-circuits to exit 0 in
                # that case; harmless once tests exist. See gh#27.
                if self.run_step(
                    "Frontend Tests",
