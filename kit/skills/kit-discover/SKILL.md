...
- **Expected-absent** (not a Gap) — a `kit-tools.md` item missing from `.claude/kit-manifest.txt` was intentionally not synced for this project, e.g. the DB recipes `migrate`/`prepare-sqlx`/`clean-db` and the `reviewer-sql` agent when `kit.config.json` sets `"database": false`. Confirm against the manifest before classifying any item as a Gap
...
4. **Separate sync gaps from CLAUDE.md gaps** — if the manifest lists a path that isn't on disk, that's a sync problem (re-run `just sync-kit`), not a documentation problem. Surface it in its own section. The same separation applies to a stale bootstrap: it is a project-state recovery, not a CLAUDE.md issue. Conversely, a `kit-tools.md` item absent from the manifest is intentionally excluded (e.g. DB artifacts when `kit.config.json` sets `"database": false`) — expected-absent, never a Gap.
