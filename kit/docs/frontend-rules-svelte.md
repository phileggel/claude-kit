...
**F0** — The frontend source tree MUST follow this layout. Four buckets (`features/`, `shell/`, `ui/`, `infra/`) carry the per-bucket include/reject discipline (see **F28**); framework-resource folders (`assets/`, `styles/`, `public/`) sit outside the bucket discipline per the Vite convention; root-level singletons (`App.svelte`, `Router.svelte`, `main.ts`, `bindings.ts`) live at `src/` root per the Svelte/Vite-ecosystem convention (`create vite@latest --template svelte-ts` and SvelteKit alike place `App.svelte` / app root at the routed-dir root). `App.svelte` is the provider tree / app entry (i18n setup, theme, error boundaries); `shell/AppShell.svelte` is the layout chrome (Mantine-style precedent for the bucket name). Splitting the two — chrome in `shell/`, provider tree at root — keeps each bucket honest to its ecosystem anchor. The DDD `application/domain/infrastructure` trio does NOT appear here — frontend features are UI surfaces, not bounded contexts (see F23, F26). This is the FE counterpart to the backend `B0` gold layout.
...
│   └── AppShell.svelte                ← persistent header / sidebar / content slot / app-level overlay hosts
│
...
│   └── index.css
│
├── App.svelte                         ← provider tree / app entry — i18n setup, theme, error boundaries (Vite convention)
├── Router.svelte                      ← cross-feature navigation wiring (F23) — at root for parity with App.svelte
...
- `Router.svelte` — registers page-level components (single wiring point)
