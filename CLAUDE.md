...
*Use for: Bug fixes, dependency updates, minor maintenance (no new business rules or features).*
...
- **Project Name Neutrality:** Agent files MUST NOT reference a specific project name (e.g., "PortfolioManager").
  ...
  - *Why it's critical:* Agents are reusable; embedding project names creates stale references when copied or renamed.
...
- **Tool Minimality:** Agent `tools:` fields should only list necessary tools. Review-only agents should not have `Edit` or `Write`.
  ...
  - *Why it's critical:* Over-privileged agents are slower and pose a security risk.
