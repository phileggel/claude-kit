...
- [ ] Generic agents: neutralize Tauri-specific fallback paths in `retro-spec.md`, `feature-planner.md`, `spec-checker.md` (fallbacks reference `src-tauri/`); fix `spec-checker.md:76` `#[tauri::command]` hardcode in contract compliance step
- [ ] `spec-diff` skill: replace hardcoded `src-tauri/` grep path with multi-path discovery (add `server/` to search list; update example output paths)

## Improvements

- [ ] `web.just`: add `format` recipe (currently missing — `just format` breaks for all web-profile projects)
